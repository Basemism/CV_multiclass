...
class CLIPSegmentationNet(nn.Module):
    """
    A UNet-style segmentation network that takes fixed CLIP image features as input.
    The input channel dimension equals the CLIP feature dimension.
    """
    def __init__(self, clip_feature_dim, num_classes):
        super(CLIPSegmentationNet, self).__init__()
        # Input convolution block: from CLIP features to a low-level feature space.
        self.inc = DoubleConv(clip_feature_dim, 16)
        ...
        # Final output layer: produces pixel-wise class logits.
